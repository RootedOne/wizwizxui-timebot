...
if(preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    for($i = 1; $i<= $text; $i++){
        ...
        if($inbound_id == 0){                    
            if($serverType == "marzban"){
                ...
                if(!$response->success){
                    if(isset($response->msg) && $response->msg == "User already exists"){ // Check if msg property exists
                ...
                if(!$response->success){
                    if(isset($response->msg) && strstr($response->msg, "Duplicate email")) $remark .= RandomString(); // Check if msg property exists
                    elseif(isset($response->msg) && strstr($response->msg, "Port already exists")) $port = rand(1111,65000); // Check if msg property exists
	...
	if(isset($response->msg) && $response->msg == "inbound not Found"){ // Check if msg property exists
            sendMessage("❌ | 🥺 سطر (inbound) با آیدی $inbound_id تو این سرور وجود نداره ، مدیر رو در جریان بزار ...");
            break;
    	}
    	if(!$response->success){
            $errorMsg = isset($response->msg)?$response->msg:"خطای نامشخص";
            sendMessage('❌ | 😮 وای خطا داد لطفا سریع به مدیر بگو ...');
            sendMessage("خطای سرور {$serverInfo['title']}:\n\n" . $errorMsg, null, null, $admin);
            break;
        }
    
        if($serverType == "marzban"){
            $marzbanUserObj = $response->obj;
            $uniqid = $token = str_replace("/sub/", "", $marzbanUserObj->subscription_url); // Corrected: use obj for Marzban response
            $subLink = $botState['subLinkState'] == "on"?$panelUrl . $marzbanUserObj->subscription_url:""; // Corrected
            $vraylink = $marzbanUserObj->links; // Corrected
            $vray_link = json_encode($marzbanUserObj->links); // Corrected
        }
        else{
...
if(preg_match('/havePaiedWeSwap(.*)/',$data,$match)) {
    ...
    for($i = 1; $i <= $accountCount; $i++){
        ...
        if($serverType == "marzban"){
            $marzbanUserObj = $response->obj;
            $uniqid = $token = str_replace("/sub/", "", $marzbanUserObj->subscription_url);
            $subLink = $botState['subLinkState'] == "on"?$panelUrl . $marzbanUserObj->subscription_url:"";
            $vraylink = $marzbanUserObj->links;
            $vray_link = json_encode($marzbanUserObj->links);
        }else{
            $token = RandomString(30);
            $subLink = $botState['subLinkState']=="on"?$botUrl . "settings/subLink.php?token=" . $token:"";
...
if($userInfo['step'] == "showAccount" and $text != $buttonValues['cancel']){
    $original_text = $text; // Keep original text for Marzban link search
    $uuid_to_search = $text; // This will be the UUID for X-UI

    if(preg_match('/^vmess:\/\/(.*)/',$text,$match)){
        $decoded_vmess = json_decode(base64_decode($match[1]),true);
        if (isset($decoded_vmess['id'])) {
            $uuid_to_search = $decoded_vmess['id'];
        }
        // $marzbanText is already $original_text
    }elseif(preg_match('/^vless:\/\/(.*?)\@/',$text,$match)){
        $uuid_to_search = $match[1];
        // $marzbanText is already $original_text
    }elseif(preg_match('/^trojan:\/\/(.*?)\@/',$text,$match)){
        $uuid_to_search = $match[1];
        // $marzbanText is already $original_text
    }elseif(!preg_match('/[a-f0-9]{8}\-[a-f0-9]{4}\-4[a-f0-9]{3}\-(8|9|a|b)[a-f0-9]{3}\-[a-f0-9]{12}/i', $uuid_to_search)){
        // If it's not a standard UUID after potential parsing, and not a link, it might be an error or direct Marzban username.
        // We proceed, as Marzban usernames don't have to be UUIDs.
        // If it's not a valid format for any panel, it will simply not be found.
    }
    $uuid_to_search = htmlspecialchars(stripslashes(trim($uuid_to_search)));
    $marzban_text_search = htmlspecialchars(stripslashes(trim($original_text)));
    ...
    $found = false;
    $config_details = []; // To store extracted details

    while($server_row = $serversList->fetch_assoc()){
        $serverId = $server_row['id'];
        $serverType = $server_row['type'];
        $panelUrl = $server_row['panel_url']; // For Marzban sub link construction

        if($serverType == "marzban"){
            $marzbanUserResponse = getMarzbanUser($serverId, $uuid_to_search); // Try with parsed UUID as username first
            if(!$marzbanUserResponse && !preg_match('/[a-f0-9]{8}\-[a-f0-9]{4}\-4[a-f0-9]{3}\-(8|9|a|b)[a-f0-9]{3}\-[a-f0-9]{12}/i', $uuid_to_search) && $uuid_to_search !== $marzban_text_search) {
                 // If initial search (maybe with a parsed UUID) fails, and original text was not a UUID, try original text as username.
                 // This handles cases where a direct Marzban username (not a link) is entered.
                $marzbanUserResponse = getMarzbanUser($serverId, $marzban_text_search);
            }

            if($marzbanUserResponse && $marzbanUserResponse->success && isset($marzbanUserResponse->obj)) {
                $user = $marzbanUserResponse->obj;
                // Check if the provided text (original or parsed) matches any link or the username
                $is_match = ($user->username == $uuid_to_search || $user->username == $marzban_text_search);
                if (!$is_match && isset($user->links) && is_array($user->links)) {
                    foreach ($user->links as $link) {
                        if (strpos($link, $marzban_text_search) !== false) {
                            $is_match = true;
                            break;
                        }
                    }
                }
                if (!$is_match && isset($user->subscription_url) && strpos($user->subscription_url, $marzban_text_search) !== false) {
                    $is_match = true;
                }
                ...
                if ($is_match) {
                    $found = true;
                    $config_details['isMarzban'] = true;
                    $config_details['remark'] = $user->username;
                    $config_details['total'] = $user->data_limit != 0 ? sumerize($user->data_limit) : "نامحدود";
                    $config_details['totalUsed'] = sumerize($user->used_traffic);
                    $config_details['state'] = $user->status == "active" ? $buttonValues['active'] : $buttonValues['deactive'];
                    $config_details['expiryTime'] = $user->expire != 0 ? jdate("Y-m-d H:i:s", $user->expire) : "نامحدود";
                    $leftMb_val = $user->data_limit != 0 ? ($user->data_limit - $user->used_traffic) : "نامحدود";
                    if (is_numeric($leftMb_val)) {
                        $config_details['leftMb'] = $leftMb_val < 0 ? sumerize(0) : sumerize($leftMb_val);
                    } else {
                        $config_details['leftMb'] = "نامحدود";
                    }
                    $expiryDay_val = $user->expire != 0 ? floor(($user->expire - time()) / (60 * 60 * 24)) : "نامحدود";
                    if(is_numeric($expiryDay_val)) {
                        $config_details['expiryDay'] = $expiryDay_val < 0 ? 0 : $expiryDay_val;
                    } else {
                        $config_details['expiryDay'] = "نامحدود";
                    }
                    $config_details['upload'] = "-"; // Marzban API might not provide separate up/down for a single user object easily
                    $config_details['download'] = "-";
                    $config_details['configLocation'] = json_encode(["remark" => $user->username, "uuid" => $user->username, "marzban" => true], 488);
                    $config_details['serverId'] = $serverId;
                    break;
                }
            }
             // If not found by username, try scanning all users (less efficient, might be needed if only link is provided)
            if(!$found){
                $marzbanAllUsersResponse = getMarzbanJson($serverId, "/api/users"); // Assuming getMarzbanJson can take a path
                if($marzbanAllUsersResponse && $marzbanAllUsersResponse->success && isset($marzbanAllUsersResponse->obj) && isset($marzbanAllUsersResponse->obj->users)){
                    foreach($marzbanAllUsersResponse->obj->users as $user){
                        $is_match = false;
                        if (isset($user->links) && is_array($user->links)) {
                            foreach ($user->links as $link) {
                                if (strpos($link, $marzban_text_search) !== false) {
                                    $is_match = true;
                                    break;
                        ...
                        if ($is_match) {
                            ...
                            $config_details['upload'] = "-";
                            $config_details['download'] = "-";
                            $config_details['configLocation'] = json_encode(["remark" => $user->username, "uuid" => $user->username, "marzban" => true], 488); // Store username as uuid for Marzban context
                            $config_details['serverId'] = $serverId;
                            break; // Found the user
                        }
                    }
                    if($found) break; // Break from server loop
                }
            }

        } else { // X-UI Panel
            $apiResponse = getJson($serverId);
            if($apiResponse && $apiResponse->success && isset($apiResponse->obj)){
                $list = $apiResponse->obj;
                foreach($list as $inbound_data){
                    // Case 1: Inbound with clientStats (multiple clients under one inbound setting)
                    if(isset($inbound_data->clientStats) && is_array($inbound_data->clientStats)){
                        $clientsSettings = json_decode($inbound_data->settings, true)['clients'] ?? [];
                        $clientEmail = null;
                        foreach($clientsSettings as $clientSetting){
                            if(isset($clientSetting['id']) && $clientSetting['id'] == $uuid_to_search){
                                $clientEmail = $clientSetting['email'] ?? ('client_' . $uuid_to_search);
                                break;
                            }
                        }
                        if($clientEmail){
                            foreach($inbound_data->clientStats as $stat){
                                if(isset($stat->email) && $stat->email == $clientEmail){
                                    $found = true;
                                    $config_details['isMarzban'] = false;
                                    $config_details['remark'] = $stat->email;
                                    $config_details['upload'] = sumerize($stat->up);
                                    $config_details['download'] = sumerize($stat->down);
                                    $config_details['totalUsed'] = sumerize($stat->up + $stat->down);
                                    // Total and Expiry might come from clientStat or fallback to inbound's main settings
                                    $stat_total = $stat->total ?? 0;
                                    $inbound_total = $inbound_data->total ?? 0;
                                    $actual_total = $stat_total ?: $inbound_total;
                                    $config_details['total'] = $actual_total != 0 ? sumerize($actual_total) : "نامحدود";

                                    $stat_expiry = $stat->expiryTime ?? 0;
                                    $inbound_expiry = $inbound_data->expiryTime ?? 0;
                                    $actual_expiry = $stat_expiry ?: $inbound_expiry;
                                    $config_details['expiryTime'] = $actual_expiry != 0 ? jdate("Y-m-d H:i:s", substr($actual_expiry, 0, -3)) : "نامحدود";
                                    ...
                                    $config_details['state'] = ($stat->enable ?? $inbound_data->enable ?? false) ? $buttonValues['active'] : $buttonValues['deactive'];
                                    $config_details['configLocation'] = json_encode(["id" => $inbound_data->id, "remark" => $stat->email, "uuid" => $uuid_to_search], 488);
                                    $config_details['serverId'] = $serverId;
                    ...
                    // Case 2: Direct client on an inbound (no clientStats or clientStats is not an array)
                    elseif(isset($inbound_data->settings)) {
                         ...
                         if($settingsDecoded !== null && isset($settingsDecoded->clients) && isset($settingsDecoded->clients[0])){
                             ...
                             if((isset($client->id) && $client->id == $uuid_to_search) || (isset($client->password) && $client->password == $uuid_to_search)){
                                $found = true;
                                $config_details['isMarzban'] = false;
                                $config_details['remark'] = $inbound_data->remark;
                                $config_details['upload'] = sumerize($inbound_data->up);
                                $config_details['download'] = sumerize($inbound_data->down);
                                $config_details['totalUsed'] = sumerize($inbound_data->up + $inbound_data->down);
                                $config_details['total'] = $inbound_data->total != 0 ? sumerize($inbound_data->total) : "نامحدود";
                                $config_details['expiryTime'] = $inbound_data->expiryTime != 0 ? jdate("Y-m-d H:i:s", substr($inbound_data->expiryTime, 0, -3)) : "نامحدود";
                                ...
                                $config_details['state'] = ($inbound_data->enable ?? false) ? $buttonValues['active'] : $buttonValues['deactive'];
                                $config_details['configLocation'] = json_encode(["remark"=> $inbound_data->remark, "uuid" => $uuid_to_search, "id" => $inbound_data->id], 488); // id here is inbound id
                                $config_details['serverId'] = $serverId;
                                break; // Break from server loop
                             }
                         }
                    }
                }
    ...
    if(!$found){
        ...
        $keyboard_array = [
            [
                ['text'=> $config_details['state'] ?? " ","callback_data"=>"wizwizch"],
                ['text'=>"🔘 وضعیت اکانت 🔘",'callback_data'=>"wizwizch"],
            ],
            [
                ['text'=> $config_details['remark'] ?? " ","callback_data"=>"wizwizch"],
                ['text'=>"« نام اکانت »",'callback_data'=>"wizwizch"],
            ]
        ];

        if(!$config_details['isMarzban']){
            $keyboard_array[] = [
                ['text'=> $config_details['upload'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"√ آپلود √",'callback_data'=>"wizwizch"],
            ];
            $keyboard_array[] = [
                ['text'=> $config_details['download'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"√ دانلود √",'callback_data'=>"wizwizch"],
            ];
        } else { // For Marzban, show combined usage
            $keyboard_array[] = [
                ['text'=> $config_details['totalUsed'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"√ آپلود + دانلود √",'callback_data'=>"wizwizch"],
            ];
        }

        $keyboard_array = array_merge($keyboard_array, [
            [
                ['text'=> $config_details['total'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"† حجم کلی †",'callback_data'=>"wizwizch"],
            ],
            [
                ['text'=> $config_details['leftMb'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"~ حجم باقیمانده ~",'callback_data'=>"wizwizch"],
            ],
            [
                ['text'=> $config_details['expiryTime'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"تاریخ اتمام",'callback_data'=>"wizwizch"],
            ],
            [
                ['text'=> $config_details['expiryDay'] ?? " ", 'callback_data'=>"wizwizch"],
                ['text'=>"تعداد روز باقیمانده",'callback_data'=>"wizwizch"],
            ]
        ]);

        $current_server_id_for_buttons = $config_details['serverId'];
        if ($botState['renewAccountState'] == "on") {
            $keyboard_array[] = [['text'=>$buttonValues['renew_config'],'callback_data'=>"sConfigRenew" . $current_server_id_for_buttons]];
        }
        if ($botState['updateConfigLinkState'] == "on") {
             $keyboard_array[] = [['text'=>$buttonValues['update_config_connection'],'callback_data'=>"sConfigUpdate" . $current_server_id_for_buttons]];
        }

        $keyboard_array[] = [['text'=>"صفحه اصلی",'callback_data'=>"mainMenu"]];

        $keys = json_encode(['inline_keyboard'=> $keyboard_array]);
        setUser($config_details['configLocation'], "temp");
...
if(preg_match('/sConfigRenew(\d+)/', $data,$match)){
    ...
    $inboundId = $configInfo['id']??0; // Assuming 'id' might be the key for inbound ID in temp
    $uuid = $configInfo['uuid'];
    $remark = $configInfo['remark'];
    ...
    if(isset($configInfo['marzban'])){
        ...
        $apiResponse = getJson($server_id);
        if(!$apiResponse || !isset($apiResponse->success) || !$apiResponse->success || !isset($apiResponse->obj)){
            delMessage();
            sendMessage("Error fetching server data or server data is not in expected format.");
            exit();
        }
        ...
        if($inboundId == 0){
            foreach($response as $row){
                $settingsDecoded = json_decode($row->settings);
                if ($settingsDecoded === null && json_last_error() !== JSON_ERROR_NONE) {
                    sendMessage("Error decoding settings for a row.");
                    continue;
                }

                $clients = $settingsDecoded->clients ?? null;
                if($clients === null) continue;

                if(isset($clients[0]) && ($clients[0]->id == $uuid || (isset($clients[0]->password) && $clients[0]->password == $uuid))) {
                    $port = $row->port;
                    $protocol = $row->protocol;
                    $streamSettingsDecoded = json_decode($row->streamSettings);
                     if ($streamSettingsDecoded === null && json_last_error() !== JSON_ERROR_NONE) {
                        sendMessage("Error decoding stream settings for a row.");
                        continue;
                    }
                    $configReality = ($streamSettingsDecoded->security ?? null) == "reality"?"true":"false";
                    break;
                }
            }
            ...
            foreach($response as $row){
                if($row->id == $inboundId) {
                    ...
                    $streamSettingsDecoded = json_decode($row->streamSettings);
                    if ($streamSettingsDecoded === null && json_last_error() !== JSON_ERROR_NONE) {
                        sendMessage("Error decoding stream settings for a row with specific inboundId.");
                        continue;
                    }
                    $configReality = ($streamSettingsDecoded->security ?? null) == "reality"?"true":"false";
                    break;
        ...
        if (!isset($protocol)) {
            sendMessage("Protocol could not be determined for the given UUID/InboundId.");
            exit();
        }
    ...
    if($plans->num_rows > 0){
        ...
        while($file = $plans->fetch_assoc()){
            ...
            if(isset($configInfo['marzban'])) $add = true;
                ...
                if(!isset($configReality)){
                     sendMessage("Configuration reality status could not be determined.");
                     continue;
                }
                $stmt_config = $connection->prepare("SELECT * FROM `server_config` WHERE `id` = ?");
                $stmt_config->bind_param("i", $server_id);
                $stmt_config->execute();
                $server_config_result = $stmt_config->get_result();
                if ($server_config_result->num_rows > 0) {
                    $isReality = $server_config_result->fetch_assoc()['reality'];
            ...
            if($add){
                ...
                $price = $file['price'];
                $price = ($price == 0) ? 'رایگان' : number_format($price).' تومان ';
                $currentInboundId = $configInfo['marzban'] ? 0 : ($inboundId ?? 0);
                $keyboard[] = ['text' => "$name - $price", 'callback_data' => "sConfigRenewPlan{$id}_{$currentInboundId}"];
            }
        }
        if (empty($keyboard)) {
            sendMessage("متاسفانه پلنی مطابق با شرایط کانفیگ شما یافت نشد.");
            exit();
        }
        $keyboard[] = ['text' => $buttonValues['back_to_main'], 'callback_data' => "mainMenu"];
        $keyboard = array_chunk($keyboard,1);
...
if(preg_match('/sConfigUpdate(\d+)/', $data,$match)){
    ...
    if(isset($configInfo['marzban'])){
        $marzbanUserInfo = getMarzbanUser($server_id, $remark); // Changed to getMarzbanUser
        if(!$marzbanUserInfo || !isset($marzbanUserInfo->success) || !$marzbanUserInfo->success || !isset($marzbanUserInfo->obj)){
            sendMessage("Failed to fetch Marzban user info or info not in expected format.");
            exit();
        }
        $info = $marzbanUserInfo->obj; // Use the 'obj' property
        $vraylink = $info->links; // Assuming links are directly available in the object
    }else{
        $apiResponse = getJson($server_id); // Call to X-UI
        if(!$apiResponse || !isset($apiResponse->success) || !$apiResponse->success || !isset($apiResponse->obj)){
            delMessage();
            sendMessage("Error fetching server data or server data is not in expected format for X-UI.");
            exit();
        }
        $response = $apiResponse->obj; // Use the 'obj' property

        if($response == null){delMessage(); exit();} // This might be redundant now
...
if(preg_match('/changAccountConnectionLink(\d+)/', $data,$match)){
    alert($mainValues['please_wait_message']);
    $oid = $match[1]; // This is the order_list id

    $stmt_order_lookup = $connection->prepare("SELECT * FROM `orders_list` WHERE `id`=?");
    $stmt_order_lookup->bind_param("i", $oid);
    $stmt_order_lookup->execute();
    $order_res = $stmt_order_lookup->get_result();
    if($order_res->num_rows == 0){
        sendMessage("Order details not found for ID: $oid");
        exit();
    }
    $order = $order_res->fetch_assoc();
    $stmt_order_lookup->close();

    $remark = $order['remark'];
    $uuid = $order['uuid']??"0"; // Current UUID
    $inboundId = $order['inbound_id']; // This is the inbound_id from orders_list, could be 0
    $server_id = $order['server_id'];
    $rahgozar = $order['rahgozar'];
    $file_id = $order['fileid']; // Plan ID from server_plans used to create this config

    // Fetch plan details for customPath, customPort, customSni
    $customPath = null;
    $customPort = null;
    $customSni = null;
    if ($file_id) {
        $stmt_plan_details = $connection->prepare("SELECT * FROM `server_plans` WHERE `id`=?");
        $stmt_plan_details->bind_param("i", $file_id);
        $stmt_plan_details->execute();
        $plan_details_res = $stmt_plan_details->get_result();
        if ($plan_details_res->num_rows > 0) {
            $plan_info = $plan_details_res->fetch_assoc();
            $customPath = $plan_info['custom_path'];
            $customPort = $plan_info['custom_port'];
            $customSni = $plan_info['custom_sni'];
        }
        $stmt_plan_details->close();
    }

    $stmt_server_cfg = $connection->prepare("SELECT * FROM server_config WHERE id=?");
    $stmt_server_cfg->bind_param("i", $server_id);
    $stmt_server_cfg->execute();
    $server_cfg_res = $stmt_server_cfg->get_result();
    if ($server_cfg_res->num_rows == 0) {
        sendMessage("Server configuration not found for server ID: $server_id");
        exit();
    }
    $server_info_config = $server_cfg_res->fetch_assoc(); // Renamed to avoid conflict with $server_info from other contexts
    $stmt_server_cfg->close();

    $serverType = $server_info_config['type'];
    $newUuid = null;
    $newToken = null;
    $vraylink = null;

    if($serverType == "marzban"){
        $res_marzban_renew = renewMarzbanUUID($server_id, $remark); // API Call
        if(!$res_marzban_renew || !isset($res_marzban_renew->success) || !$res_marzban_renew->success || !isset($res_marzban_renew->obj)){
            sendMessage("Failed to renew Marzban UUID. " . ($res_marzban_renew->msg ?? "No message from panel."));
            exit();
        }
        $marzbanRenewObj = $res_marzban_renew->obj;
        $vraylink = $marzbanRenewObj->links; // This is an array
        $newUuid = str_replace("/sub/", "", $marzbanRenewObj->subscription_url);
        $newToken = $newUuid;
    }else{ // X-UI Path
        $apiScanResponse = getJson($server_id);
        if(!$apiScanResponse || !isset($apiScanResponse->success) || !$apiScanResponse->success || !isset($apiScanResponse->obj)){
            sendMessage("Failed to fetch server data (for scanning) from X-UI. " . ($apiScanResponse->msg ?? "No message from panel."));
            exit();
        }
        $panel_inbounds_data = $apiScanResponse->obj;

        $port = null;
        $protocol = null;
        $netType = null;
        $panel_inbound_id_for_renewal = null;

        if($inboundId == 0){
            foreach($panel_inbounds_data as $panel_inbound){
                $settingsDecoded = json_decode($panel_inbound->settings);
                if ($settingsDecoded === null) continue;
                $clients = $settingsDecoded->clients ?? null;
                if($clients === null || !isset($clients[0])) continue;

                if($clients[0]->id == $uuid || (isset($clients[0]->password) && $clients[0]->password == $uuid)) {
                    $port = $panel_inbound->port;
                    $protocol = $panel_inbound->protocol;
                    $panel_inbound_id_for_renewal = $panel_inbound->id;
                    $streamSettingsDecoded = json_decode($panel_inbound->streamSettings);
                    if ($streamSettingsDecoded !== null) $netType = $streamSettingsDecoded->network ?? null;
                    break;
                }
            }
            if($panel_inbound_id_for_renewal === null){
                sendMessage("Could not find the specified config (UUID: $uuid) on a dedicated inbound on server ID: $server_id.");
                exit();
            }
            // For renewing a dedicated inbound, use its actual ID from the panel
            $update_response = renewInboundUuid($server_id, $panel_inbound_id_for_renewal);
        }else{
            foreach($panel_inbounds_data as $panel_inbound){
                if($panel_inbound->id == $inboundId) {
                    $port = $panel_inbound->port;
                    $protocol = $panel_inbound->protocol;
                    $streamSettingsDecoded = json_decode($panel_inbound->streamSettings);
                    if ($streamSettingsDecoded !== null) $netType = $streamSettingsDecoded->network ?? null;
            ...
            if($protocol === null) {
                sendMessage("Could not find the shared inbound with ID: $inboundId on server ID: $server_id.");
                exit();
            }
            // For renewing a client under a shared inbound, $inboundId is the shared inbound's panel ID.
            $update_response = renewClientUuid($server_id, $inboundId, $uuid);
        }

        if(!$update_response || !isset($update_response->success) || !$update_response->success || !isset($update_response->obj) || !isset($update_response->obj->newUuid)){
            sendMessage("Failed to renew UUID via X-UI. " . ($update_response->msg ?? "No message from panel."));
            exit();
        }
        $newUuid = $update_response->obj->newUuid;

        if ($protocol === null || $port === null || $netType === null) {
            sendMessage("Could not determine all necessary parameters (protocol, port, netType) to regenerate the link after UUID renewal.");
            exit();
        }
    ...
    if (!isset($vraylink) || $vraylink === null) {
        sendMessage("Fatal error: Connection links array is null after renewal process.");
        exit();
    }

    $vray_link_json = json_encode($vraylink);
    $stmt_db_update = $connection->prepare("UPDATE `orders_list` SET `link`=?, `uuid` = ?, `token` = ? WHERE `id`=?");
    $stmt_db_update->bind_param("sssi", $vray_link_json, $newUuid, $newToken, $oid);
    $stmt_db_update->execute();
    // It's good practice to check $stmt_db_update->affected_rows here.
    $stmt_db_update->close();

    $keys_display = getOrderDetailKeys($from_id, $oid);
    if ($keys_display && isset($keys_display['msg']) && isset($keys_display['keyboard'])) {
        editText($message_id, $keys_display['msg'], $keys_display['keyboard'],"HTML");
    } else {
        sendMessage("Connection link renewed successfully. Could not refresh display automatically for order ID $oid.");
    }
}
if(preg_match('/changeUserConfigState(\d+)/', $data,$match)){
    alert($mainValues['please_wait_message']);
    $oid = $match[1]; // order_list id

    $stmt_order = $connection->prepare("SELECT * FROM `orders_list` WHERE `id`=?");
    $stmt_order->bind_param("i", $oid);
    $stmt_order->execute();
    $order_result = $stmt_order->get_result();
    if($order_result->num_rows == 0){
        sendMessage("Order not found for ID: $oid");
        exit();
    }
    $order = $order_result->fetch_assoc();
    $stmt_order->close();

    // $userId = $order['userid']; // Not directly used here but good for context
    $uuid = $order['uuid'] ?? "0";
    $inboundId = $order['inbound_id']; // This is from orders_list, could be 0
    $server_id = $order['server_id'];
    $remark = $order['remark'];
    ...
    $stmt_server_cfg->bind_param("i", $server_id);
    $stmt_server_cfg->execute();
    $server_cfg_result = $stmt_server_cfg->get_result();
    if($server_cfg_result->num_rows == 0){
        sendMessage("Server configuration not found for server ID: $server_id");
        exit();
    }
    $server_config_details = $server_cfg_result->fetch_assoc(); // Renamed
    $stmt_server_cfg->close();

    $serverType = $server_config_details['type'];
    $update_response = null;

    if($inboundId == 0){ // Target is a main inbound (dedicated user)
        if($serverType == "marzban") {
            $update_response = changeMarzbanState($server_id, $remark); // API Call
        } else { // X-UI dedicated inbound
            // To change state of a dedicated inbound, we need its actual panel ID, not just UUID
            // This requires scanning the panel like in other functions
            $apiScanResponse = getJson($server_id);
            if(!$apiScanResponse || !isset($apiScanResponse->success) || !$apiScanResponse->success || !isset($apiScanResponse->obj)){
                sendMessage("Failed to fetch server data (for state change scanning) from X-UI. " . ($apiScanResponse->msg ?? ""));
                exit();
            }
            $panel_inbounds_data = $apiScanResponse->obj;
            $panel_inbound_id_for_state_change = null;
            foreach($panel_inbounds_data as $panel_inbound){
                $settingsDecoded = json_decode($panel_inbound->settings);
                if ($settingsDecoded === null) continue;
                $clients = $settingsDecoded->clients ?? null;
                ...
                // Assuming the UUID stored in orders_list.uuid is the client's ID/password within the inbound settings
                if(isset($clients[0]) && ($clients[0]->id == $uuid || (isset($clients[0]->password) && $clients[0]->password == $uuid))) {
                    $panel_inbound_id_for_state_change = $panel_inbound->id; // This is the actual inbound ID on the panel
                    break;
                }
            }
            if($panel_inbound_id_for_state_change === null){
                 sendMessage("Could not find the X-UI inbound associated with UUID $uuid to change its state.");
                 exit();
            }
            $update_response = changeInboundState($server_id, $panel_inbound_id_for_state_change); // API Call, using panel's inbound ID
        }
    }else{ // Target is a client under a shared inbound (orders_list.inbound_id is the shared inbound's panel ID)
        // For X-UI, $inboundId from orders_list is the correct panel ID of the shared inbound.
        $update_response = changeClientState($server_id, $inboundId, $uuid); // API Call
    }

    if($update_response && isset($update_response->success) && $update_response->success){
        // alert($mainValues['please_wait_message']); // Already alerted at the beginning
        $keys = getUserOrderDetailKeys($oid); // $oid is $match[1]
        if ($keys && isset($keys['msg']) && isset($keys['keyboard'])) {
            editText($message_id, $keys['msg'], $keys['keyboard'], "HTML");
        } else {
             sendMessage("State changed successfully. Could not refresh display for order ID $oid.");
        }
    } else {
        $error_message = "عملیه مورد نظر با مشکل روبرو شد";
        if ($update_response && isset($update_response->msg)) {
            $error_message .= "\nPanel message: " . $update_response->msg;
        } elseif (is_null($update_response)) {
            $error_message .= "\nNo response from the panel/API function.";
        } else {
            $error_message .= "\nResponse from panel was not successful or was malformed.";
        }
...
if(preg_match('/approveRenewAcc(.*)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
	...
	if(is_null($response) || !$response->success){
	    $errorMsg = (!is_null($response) && isset($response->msg))?$response->msg:"نات مشخص";
		alert('🔻مشکل فنی در اتصال به سرور (' . $errorMsg . '). لطفا به مدیریت اطلاع بدید',true);
		exit;
	}
	$stmt = $connection->prepare("UPDATE `orders_list` SET `expire_date` = ?, `notif` = 0 WHERE `id` = ?");
	$newExpire = $time + $days * 86400;
